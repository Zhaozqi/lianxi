...
class Site extends IController
{
	...
	function index()
	{
		...
		$ip='114.114.114.114';
		// 调用百度接口，向其传递IP地址，接口会返回此IP对应的城市
		$ak='ugkyTppsqCinW4KN4YtLt2y3Oodg8sN7';
		$url="http://api.map.baidu.com/location/ip?ip=$ip&ak=$ak";
		// 调用 CURL request 方法，发送请求，获取接口返回的数据
		$str=$this->request($url);
		//$str='{"address":"CN|\u6c5f\u82cf|\u5357\u4eac|None|CHINANET|0|0","content":{"address_detail":{"province":"\u6c5f\u82cf\u7701","city":"\u5357\u4eac\u5e02","district":"","street":"","street_number":"","city_code":315},"address":"\u6c5f\u82cf\u7701\u5357\u4eac\u5e02","point":{"y":"3747938.29","x":"13222458.63"}},"status":0}1';
		$left=stripos($str,'{');		// 获取左侧第一个｛ 符号的位置
		$right=strrpos($str,'}');		// 获取右侧第一个 ｝ 符号的位置
		$str=substr($str,$left,$right-$left+1);		// 获取左侧第一个｛符号和右侧第1个｝符号之间的字符串，包含｛和｝
		$data=json_decode($str,true);				// 将json字符串转换成PHP数组
		$city=$data['content']['address_detail']['city'];		// 获取城市名称

		$this->city=$city;		// 将city传递到模板中
	...
	function home()
	{
		...
		// --------新增---------
		// 调用getLatLng方法，传递商家的地址，获取此地址对应的经纬度值
		$data=$this->getLatLng($sellerRow['address']);
		// 将经纬值存储到sellerRow数组中
		$sellerRow['lng']=$data['lng'];
		$sellerRow['lat']=$data['lat'];
	...
	// 编写一个方法，将商家地址转换成经纬度，之后传递到前台商家首页，在商家首页中再将此经纬度转换成地图展示出来
	public function getLatLng($address){
		$url='http://api.map.baidu.com/geocoder/v2/?address='.$address.'&output=json&ak=pMIGLKHGEQ10ibfC4YjmX3LkQaZCV0iS&callback=showLocation';
		...
		// 获取左花括号和右花括号之间的json格式数据
		//$str='showLocation&&showLocation({"status":0,"result":{"location":{"lng":116.3084202915042,"lat":40.05703033345938},"precise":1,"confidence":80,"comprehension":100,"level":"道路"}})';
		$left=strpos($str,'{');
		$right=strrpos($str,'}');
		$str=substr($str,$left,$right-$left+1);
		//echo $str;
		// 将json格式的字符串转换成PHP数组
