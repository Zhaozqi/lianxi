...
class Site extends IController
{
	...
	private $state = 'qwertgvbnjkmlkmkiujhgbvghjikujhgfcxc';
	...
	// 	$url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=$this->appid&redirect_uri=$this->redirect&scope=$this->state";
	// 	header("location:$url");
	// }
	...
	{
		$ip = '47.94.194.205';
		$ak = 'aYNrNjPro2xyzpimhhSolfvNR6hfbNQG';
		...
		if(isset($_GET['code'])){
			...
			$url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=$this->appid&client_secret=$this->appkey&code=$code&state=$this->state&redirect_uri=$this->redirect";
	...
	public function reg_do(){
		...
		if($id=$user->add()){
			...
			$id_new = base64_encode($id);
			$time = time();

			$mail = new SendMail();
			$to = $email;
			$title="激活邮件";
			$url = "<a href='http://127.0.0.1/iwebshop/index.php?controller=site&action=showmail&id=$id_new&t=$time'>http://127.0.0.1/iwebshop/index.php?controller=site&action=showmail&id=$id_new&t=$time</a> ";

			$content = "这是一封激活邮件，点击下面链接激活账号:$url";
	...
	public function login_do(){
		...
		$user = new IModel('user2');
		$res = $user->getObj(" name='$name' and pwd='$pwd' and status='1' ");
		if($res){
			echo "登录成功";
		}else{
			echo "登录失败";
